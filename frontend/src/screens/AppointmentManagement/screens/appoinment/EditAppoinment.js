...
export  const EditAppoinment = () => {
    ...
    return (
        ...
        {
            openAlert?(
                <Alert message={aleartMessage} type={aleartType} open={openAlert} setOpen={setOpenAleart}/>
            ):(
             <div className="body-container">
                <h2>Edit an Appoinment</h2>
                <div className="page-image"></div>
                <div className="form-container">
                    <div className="form-left">
                        <div className="form-input">
                            <label htmlFor="name">Name</label>
                            <input className="input" type="text" value={formData?.name} id="name" name="name" placeholder="Enter your name" onChange={onchange}/>
                            <div className='error-text'>{error.name? error.name:''}</div>
                        </div>
                        <div className="form-input">
                            <label htmlFor="mail">E-mail</label>
                            <input className="input" type="email" value={formData?.mail} id="mail" name="mail" placeholder="Enter your e-mail" onChange={onchange}/>               
                            <div className='error-text'>{error.name? error.mail:''}</div>
                        </div>
                        <div className="form-input">
                            <label htmlFor="mobile">Phone Number</label>
                            <input className="input" type="text" id="mobile" value={formData?.mobile} name="mobile" placeholder="Enter your phone number" onChange={onchange}/>
                            <div className='error-text'>{error.name? error.mobile:''}</div>
                        </div>
                        <div className="form-input">
                            <label htmlFor="doctor">Doctor</label>
                            <input className="input" type="text" id="doctor" name="doctor" value={formData?.doctor} placeholder="Enter doctor's name"onChange={onchange}/>
                    ...
                    <div className='form-input'>
                            ...
                            <input className="input-date" type="date" id="date" name="date" value={formData?.date} placeholder="Enter date" onChange={onchange}/>
                            <div className='error-text'>{error.date? error.date:''}</div>
                        </div>
                        <div className='form-input'>
                            <label htmlFor="time">Time</label>
                            <input className="input-date" type="time" id="time" name="time" value={formData?.time} placeholder="Enter time" onChange={onchange}/>
                            <div className='error-text'>{error.time? error.time:''}</div>
                        </div>
                        <div className="form-input">
                            <label htmlFor="payment">Payment Method</label>
                            <select className="input" name="payment" id="payment" onChange={onchange} defaultValue={formData?.payment}>
                        ...
                        <div className="button-row">
                            <button className="btn-submit" onClick={()=>{handleSubmit();setOpenAleart(!openAlert)}}>Submit</button>
